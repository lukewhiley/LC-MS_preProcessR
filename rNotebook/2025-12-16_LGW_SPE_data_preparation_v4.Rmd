...
#for(idx_plate in c("p041b")){ #for testing
for(idx_plate in master_list$project_details$plateList){
...
print(idx_plate)
  ...
  for(idx_mzML in master_list$data$mzR[[idx_plate]]){
    ...
    for(idx_mrm in names(master_list$data$peakRegionFinal)){
      ...
      #if multiple transition matches
        if(length(mzml_mrm) > 1){
          for(idx in mzml_mrm){
           if(! (peakApexRT > min(tempMzml$mzR_chromatogram[idx][[1]][,1]) & 
               peakApexRT < max(tempMzml$mzR_chromatogram[idx][[1]][,1]))
           ){mzml_mrm = mzml_mrm[-which(mzml_mrm == idx)]}
          }
          
          mzml_mrm = mzml_mrm[1]
        }
        
        #if not mrm funtion found set as 0 for use later
          if(length(mzml_mrm) == 0){
             mzml_mrm = 0
          }
      ...
      # if(!mzml_mrm %in% updateMrm){mzml_mrm <- updateMrm}
      # if(length(mzml_mrm) > 1){
      #   medianTime <- NULL
      #   for(idx in mzml_mrm)
      #     peakStart > tempMzml$mzR_chromatogram[idx][[1]][,1][1]
      # }
...
master_list$data$peakArea$Long$peakAreaMetabolite[which(master_list$data$peakArea$Long$peakAreaMetabolite<150000)] <- NA
