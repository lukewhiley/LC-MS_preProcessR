...
R markdown notebook for universal data processing of the SPE work
...
svDialogs::dlg_message("set up AB folder in directory as follows: ~/PROJECT_ID/AB/")
svDialogs::dlg_message("set up N folders in directory as follows: ~/PROJECT_ID/N/")
...
#.
#USER INPUT
add the project details in this next chunk
...
master_list$project_details$project_dir <-  rstudioapi::selectDirectory(); master_list$project_details$project_dir <- normalizePath(master_list$project_details$project_dir, winslash = "/")
#set SPE type
...
## prepare mzml into folders

- first place mzmls into the following folder sub-structure:

~project_name/[AB or N]/data/mzml

## rename the mzml files to put the number at the front for sorting, remember to rename at the end
...
svDialogs::dlg_message("NOTE THIS CRIPT WILL NOT WORK ON WINDOWS IF PATH LENGTH IS >260 CHARACTERS")
...
for(idx_plate in master_list$project_details$plateList){
  ...
  # reviewPlotDataAll <- reviewPlotDataAll %>%
  #   bind_rows(
  #     reviewPlotData
  #   )
...
svDialogs::dlg_message(paste0("only consider gross changes where clear error has occured. consider check solution, LTR and PQC"))
